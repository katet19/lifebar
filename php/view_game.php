...
function DisplayGame($gbid){
	...
	$users = GetUsersXPForGame($game->_id, $_SESSION['logged-in']->_id);
	$myxp = GetExperienceForUserByGame($_SESSION['logged-in']->_id, $game->_id);
	ShowGameHeader($game, $critics, $users, $myxp);
	ShowGameContent($game, $critics, $users, $myxp);
}
...
function ShowGameHeader($game, $critics, $users, $myxp){
	?>
	...
	if($_SESSION['logged-in']->_id > 0){ ?>
	    ...
	    <ul>
	    	...
	    	<?php } ?>
	      	...
	      	<li><span class="GameHiddenActionLabel">Remove from My Library</span><a class="btn-floating grey darken-1 game-remove-owned-btn" <?php if($myxp->_owned != "Yes"){ echo "style='display:none;'"; } ?> data-gameid='<?php echo $myxp->_game->_id; ?>'><i class="mdi-av-my-library-add"></i></a></li>
	      	<li><span class="GameHiddenActionLabel">Add to My Library</span><a class="btn-floating orange darken-2 game-add-owned-btn" <?php if($myxp->_owned == "Yes"){ echo "style='display:none;'"; } ?> data-gameid='<?php echo $myxp->_game->_id; ?>'><i class="mdi-av-my-library-add"></i></a></li>
	      	<?php if(sizeof($myxp->_playedxp) == 0 && $myxp->_game->_year > 0){ ?>
